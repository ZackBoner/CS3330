...
int A[1024*1024] __attribute__((aligned(128)));
int B[1024*1024] __attribute__((aligned(128)));
int C[1024*1024] __attribute__((aligned(128)));
...
#pragma clang loop interleave(disable)
     ...
     */
    for (int k_outer = 0; k_outer < 1024; k_outer += 16) {
        for (int i = 0; i < 1024; ++i) {
            for (int k = k_outer; k < k_outer + 16; ++k) {
                A[i + k] = B[k * 16 + i] * C[i * 1024 + k];
            }
        }
    }
    printf("%d", A[1026]);    
